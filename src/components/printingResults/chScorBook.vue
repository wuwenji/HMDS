      ...
      <tr class="line-tr">
        ...
        <td class="yellow">
          <div style="position: relative; width: 100%; height: 100%;">
            <textarea class="big-text" v-model="formData.modelNumber">{{formData.modelNumber}}</textarea>
          </div>
          ...
          <div class="right-bottom-div">
            ...
            <table v-if="formData.taskName !== '' && formData.taskName.indexOf('NVG') > -1" class="table table3">
              ...
              <tr v-for="(item, key) in formData.treatmentEntryList" :key="key">
                <td>
                  <div style="position: relative; width: 100%; height: 100%;">
                    <textarea class="big-text min-text" v-model="item.modelNumber" v-html="item.modelNumber">
                    </textarea>
                  </div>
                </td>
...
export default {
  ...
  created () {
    ...
    // console.log('获取值', this.orderInfo.qTemperature1)
    this.getData()
  },
  methods: {
    fileChange (e) {
